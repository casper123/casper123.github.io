...
app.factory('opService', ["$http", "$rootScope", "$state", "$ionicModal", "$ionicViewService", "$q",
    function($http, $rootScope, $state, $ionicModal, $ionicViewService, $q) {
        ...
        var ret = {
            ...
            convertActivityItem: function(arrItem) {
                var item = JSON.parse(JSON.stringify(arrItem));
                item.raw = arrItem;

                item.activityTimeDisplay = moment(arrItem.activityTime / 1000).calendar();

                if (arrItem.postPictures) {
                    item.postPictureFirstScaled = arrItem.postPictures[0].pictureUrl + self.LIST_SCALE;
                }

                if (arrItem.actUserProfilePicture) {
                    item.actUserProfilePictureScaled = arrItem.actUserProfilePicture + self.LIST_SCALE;
                }

                return item;
            },
            getDeals: function(mode, fromRankCount, fromFollowTs, fromTs) {
                var from = -1;
                if (mode == "latest") {
                    from = fromTs;
                }
            ...
            login: function(username, password) {
                var psH = CryptoJS.MD5(password);
                return $http({
                    url: self.SERVICE_HOST + '/user/login',
                    method: 'POST',
                    data: {
                        username: username,
                        password: psH.toString()
            ...
            getUserProfile: function(actUserId) {
                return $http({
                    url: self.SERVICE_HOST + '/user/profile' + (actUserId ? "/" + actUserId : ""),
                    method: 'GET'
                });
            },
            updateUserProfile: function(picture, email, mobile) {
                return $http({
                    url: self.SERVICE_HOST + '/user/profile',
                    method: 'POST',
                    data: {
                        picture: picture,
                        email: email,
            ...
            follow: function(actUserId) {
                return $http({
                    url: self.SERVICE_HOST + '/user/follow/' + actUserId,
                    method: 'POST'
                });
            },
            unfollow: function(actUserId) {
                return $http({
                    url: self.SERVICE_HOST + '/user/unfollow/' + actUserId,
                    method: 'POST'
                });
            },
            followStatus: function(actUserId) {
                return $http({
                    url: self.SERVICE_HOST + '/user/followStatus' + (actUserId ? "/" + actUserId : ""),
                    method: 'GET'
                });
            },
            ...
            getDealTags: function(query) {
                return $http({
                    url: self.SERVICE_HOST + '/deal/tags',
                    method: 'GET',
                    params: {
                        q: query
                    }
